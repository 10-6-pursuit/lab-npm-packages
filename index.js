...
function sumNumbers(array) {
  return _.compact(array).reduce((x, y) => x + y, 0);
}
...
function getUniqueClasses(collection) {
  const titlePriceArray = _.map(collection, (classObj) =>
    _.pick(classObj, ["title", "priceInCents"])
  );

  const uniqueTitlePriceArray = _.uniqBy(titlePriceArray, "title");

  return uniqueTitlePriceArray;
}
...
function orderClassesByTitleAndLevel(collection) {
  const sortedCollection = _.orderBy(
    collection,
    ["title", "level"],
    ["asc", "desc"]
  );

  const result = _.map(sortedCollection, (classObj) =>
    _.pick(classObj, ["title", "instructor", "level"])
  );
